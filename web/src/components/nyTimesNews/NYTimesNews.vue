<template lang="pug">
  .news-content
    ...
    ul.news-list(v-if="nyNewsList.length > 0")
      li(v-for="newsItem in nyNewsList" :key="newsItem.title")
...
import { getNyCategories, getSelectCateNy } from '@/service/newsAPIList/index.js';
...
export default {
  ...
  setup(props) {
    ...
    const selectNewsCategory = async (category) => {
      ...
      // wait dom update
      await nextTick();
      console.log(nyNewsList.value)
      // clean array
      if (category === 'All') {
        nyNewsList.value = [...props.nyList];
      } else {
        console.log(category)
        try {
          // Calling the specific category api
          const response = await getSelectCateNy({ category });
          console.log('response.categories', response);
          if (response && Array.isArray(response)) {
            nyNewsList.value = response;
          } else {
    ...
    const getCategoriesData = async () => {
      try {
        const categoriesData = await getNyCategories();
