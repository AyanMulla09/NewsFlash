<template lang="pug">
  .news-content
    ...
    ul.news-list(v-if="asianNewsList.length > 0")
      li(v-for="newsItem in asianNewsList" :key="newsItem.title")
...
export default {
  ...
  setup(props) {
    ...
    const selectNewsCategory = async (category) => {
      ...
      // wait dom update
      await nextTick();
      console.log(asianNewsList.value)
      // clean array
      if (category === 'All') {
        asianNewsList.value = [...props.asiaList];
      } else {
        console.log(category)
        try {
          // Calling the specific category api
          const response = await getSelectCateAsiaNews({ category });
          console.log('response.categories', response);
          if (response && Array.isArray(response)) {
            asianNewsList.value = response;
          } else {
    ...
    const getCategoriesData = async () => {
      try {
        const categoriesData = await getAsiaNewsCategories();
